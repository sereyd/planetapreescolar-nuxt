...
export default {
  data() {
    return {
      ...
      listasOpciones: [
        {
          titulo: "Semáforo",
          img: "~/static/images/iconos/semaforo.png",
          selected: false
        },
        {
          titulo: "Si/No",
          img: "~/static/images/iconos/SiNo.png",
          selected: false
        },
        {
          titulo: "Rango 1/10",
          img: "~/static/images/iconos/rango.png",
          selected: false
        },
        {
          titulo: "ON/OFF",
          img: "~/static/images/iconos/OnOff.png",
          selected: false
        }
      ],
      listasDefault: [
        {
          ...
          tipoLista: "Si/No",
          rango: ["Si", "No"],
          calificacionDefault: "Si"
        },
        {
          nombre: "Conducta",
          tipoLista: "Semáforo",
          rango: ["Bien", "Regular", "Mal"],
          calificacionDefault: "Si"
        }
      ],
      // listasFireBase:[],
  ...
  computed: {
    ...
    diasRestantes() {
        if(this.materia && this.materia.horario){
  ...
  methods: {
    ...
    updateFecha() {
      ...
      let materia = this.grupo.materias.find(
        materia => this.idGrupo === materia.nombreGrupo
      );
      this.clase = materia.clases.find(c => c.fecha === this.fecha);

      if (!this.clase) {
        this.actualizarClasesCreadas({
          clase: materia.nombreGrupo,
          fecha: this.fecha
        });
        ...
        materia = this.grupo.materias.find(
          mat => this.idGrupo === mat.nombreGrupo
        );
        this.clase = materia.clases.find(c => c.fecha === this.fecha);
    ...
    inigropdata(){
        ...
        this.idGrupo = this.datain.nombreGrupo;
        return this.idGrupo
    },
    loadgroupdetails() {
      ...
      this.materia = this.grupo.materias.find(
        materia => this.idGrupo === materia.nombreGrupo
      );
    ...
    validarFormularioAlumno() {
      ...
      if (this.esAlumnoValido) this.crearNuevoAlumno();
    },
          ...
          mat.clases.map(cla => {
            ...
            cla.listas.map(li => {
              listasGA.push({
                lista: li.nombre,
                calificacion:
                  li.tipoLista === "Si/No" || li.tipoLista === "ON/OFF"
                    ? true
                    : li.tipoLista === "Rango 1/10"
                    ? 0
                    : "Bien",
          ...
          if (c.fecha !== this.fecha) {
            c.listas.map(lista => {
              listasAlumno.push({
                lista: lista.nombre,
                calificacion:
                  lista.tipoLista === "Si/No" || lista.tipoLista === "ON/OFF"
                    ? true
                    : lista.tipoLista === "Rango 1/10"
                    ? 0
                    : "Bien",
                tipoLista: lista.tipoLista,
                rango: lista.rango
      ...
      try {
        ...
        const rango =
          tipoLista === "Semáforo"
            ...
            : tipoLista === "Si/No"
            ? this.rangoSiNo
            : tipoLista === "Rango 1/10"
            ? this.rangoNumeros
            : this.rangoOnOff;

        const calificacion =
          tipoLista === "ON/OFF" || tipoLista === "Si/No"
            ? true
            : tipoLista === "Rango 1/10"
            ? 0
            : "Bien";
    ...
    asignarCal(lista, alumno, nombreLista, index) {
      const alumnoModificar = this.clase.alumnos.find(
        a => alumno.nombre === a.nombre && alumno.apellidos === a.apellidos
      );
      ...
      alumnoModificar.listas.map(li => {
        ...
        if (li.nombre === nombreLista) {
          ...
          if (li.tipoLista === "Si/No") {
            ...
            else if (li.calificacion === "Regular") li.calificacion = "Mal";
            else if (li.calificacion === "Mal") li.calificacion = "Bien";
    ...
    cambiarHorario(tipoForm) {
      ...
      try {
        ...
        console.log("this.horarioHoy");
        console.log(this.horarioHoy);
        this.horarioHoy = this.materia.horario.find(
          hr => hr.dia === this.diaHoy
        );
        console.log("this.horarioHoy");
        console.log(this.horarioHoy);
        ...
        this.materia.horario.forEach(hr => {
          hr.edit = false;
        });
    ...
    validarFormularioHorario(ref) {
      ...
      this.esListaValido =
        ref === "Lunes"
          ...
          : ref === "Martes"
          ? this.$refs.Martes[0].validate()
          : ref === "Miercoles"
          ? this.$refs.Miercoles[0].validate()
          : ref === "Jueves"
          ? this.$refs.Jueves[0].validate()
          : ref === "Viernes"
          ? this.$refs.Viernes[0].validate()
          : ref === "Sabado"
          ? this.$refs.Sabado[0].validate()
          : ref === "Domingo"
          ? this.$refs.Domingo[0].validate()
          : this.$refs.formHorario.validate();

      // this.esListaValido =this.$refs.Martes.validate();
      console.log(this.$refs);
      if (this.esListaValido) this.cambiarHorario(tipoForm);
    }
